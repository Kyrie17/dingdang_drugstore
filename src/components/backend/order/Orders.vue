<template>
  <div>
    ...
    <el-card class="box-card">
      <el-row :gutter="20">
        <el-col :span="5">
          <el-input placeholder="请输入订单编号" prefix-icon="el-icon-s-order" v-model="queryInfo.orderSn" clearable @clear="getOrderList"></el-input>
        </el-col>
        <el-col :span="4">
      ...
      <el-table :data="orderList" stripe>
        ...
        <el-table-column label="用户姓名" prop="memberUsername"></el-table-column>
...
export default {
  ...
  created() {
    this.queryInfo.orderSn = null
    this.getOrderList();
  }, 
  methods: {
    getStatus(row, column) {
      var res = ''
      if (row.status == 0) res = '待付款'
      else if(row.status == 1) res = '待发货'
      else if(row.status == 2) res = '已发货'
      else if(row.status == 3) res = '已完成'
      else if(row.status == 4) res = '已关闭'
      else res = '无效订单'
      return res;
    },
    // 异步获取用户列表
    async getOrderList() {
      const { data: res } = await this.$http.get("order/get_order", {

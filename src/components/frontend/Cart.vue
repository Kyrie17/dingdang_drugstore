                  ...
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div class="items-choose">
                        <!-- <span class="blue-checkbox-new " :class="{'checkbox-on':item.checked === true}" @click="editCart('check',item)"></span> -->
                        <el-checkbox v-model="item.checked"  @change="editCartItemStatus('check',item)"></el-checkbox>
                      </div>
                      <!--图片-->
                      <div class="items-thumb fl">
                        <img :alt="item.productName" :src="item.productPic"><a @click="goodsDetails(item.productId)" :title="item.productName" target="_blank"></a>
                      </div>
                      <!--信息-->
              ...
              <div class="cart-bar-operation">
                <div>
                  <div class="choose-all">
                    ...
                    <el-checkbox v-model="allChecked" @change="editCheckAll">全选</el-checkbox>
                  </div>
                </div>
              </div>
              <div class="shipping">
...
//   import { mapMutations, mapState } from 'vuex'
  ...
  import YBuyNum from './common/BuyNum'
  ...
  export default {
    ...
    methods: {
      ...
      async editCheckAll () {
        ...
        params.append('allChecked', this.allChecked);
        const { data: res } = await this.$http.post('cart/update_select_all', params)
        if (res.status !== 'success') return this.$message.error('全选失败')
        this.getCartList()
      },
      // 修改购物车勾选状态
      async editCartItemStatus (type, item) {
        if (type && item) {
          // 勾选
          if (type === 'check') {
            var params = new URLSearchParams()
            ...
            const { data: res } = await this.$http.post('cart/update_cart_item', params)
            if (res.status !== 'success') return this.$message.error('更新失败')
      ...
      // 修改购物车数量
      async EditNum (productNum, productId, checked) {
        var params = new URLSearchParams()
        params.append('userId', this.userId);
        params.append('productId', productId);
        params.append('quantity', productNum);
      ...
      async cartdel (productId) {
        ...
        params.append('userId', this.userId);
        params.append('productId', productId);
        const { data: res } = await this.$http.post('cart/delete_cart_item', params)

      ...
      <y-popup :open="popupOpen" @close='popupOpen=false' :title="popupTitle">
        <div slot="content" class="md" :data-id="msg.id">
          ...
          <y-button text='保存'
                    ...
                    @btnClick="save({id:msg.id,name:msg.name,phone:msg.phone,address:msg.address,isDefault:msg.isDefault})">
          </y-button>
        </div>
      </y-popup>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取购物车
      async getCartList() {
        const { data: res } = await this.$http.get("cart/get_item", {params: {userId: this.userId}});
        if (res.status !== "success") {
          return this.$message.error("获取购物清单失败！");
        }
        this.cartList = res.data;
      },
      // 获取商品地址列表
      async getAddressList () {
        const { data: res } = await this.$http.get("address/get_address", {params: {userId: this.userId}});
        if (res.status !== "success") {
          return this.$message.error("获取用户地址失败！");
        }
        if (res.data.length) {
          this.addList = res.data
          this.addressId = this.addList[0].id || 1
          this.userName = this.addList[0].name
          this.tel = this.addList[0].phone
          this.streetName =  this.addList[0].address
        } else {
          this.addList = []
        }
      },
      ...
      async _submitOrder () {
        ...
        const { data: res } = await this.$http.post("order/add_order", params);
        if (res.status !== "success") {
      ...
      async save (p) {
        ...
        if (p.id) {
          ...
          params.append('phone', p.phone);
          params.append('address', p.address);
          params.append('isDefault', p.isDefault);
          params.append('memberId', this.userId);
          const { data: res } = await this.$http.post("address/update_address", params);
          if (res.status !== "success") {
            return this.$message.error("更新用户地址失败！");
          }
        } else {
          var params = new URLSearchParams()
          params.append('name', p.name);
          ...
          params.append('isDefault', p.isDefault);
          params.append('memberId', this.userId);
          const { data: res } = await this.$http.post("address/add_address", params);
          if (res.status !== "success") {
            return this.$message.error("添加用户地址失败！");
          }
        }
        this.getAddressList()
      },
      ...
      async del (addressId) {
        ...
        const { data: res } = await this.$http.post("address/delete_address", params);
        if (res.status !== "success") {
          return this.$message.error("删除用户地址失败！");
        }
        this.getAddressList()
      },
